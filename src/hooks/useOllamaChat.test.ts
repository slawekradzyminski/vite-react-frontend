...
describe('useOllamaChat', () => {
  ...
  it('maintains conversation history through multiple messages', async () => {
    ...
    const mockResponse1 = new Response(
      'data: {"model":"qwen3.5:2b","message":{"role":"assistant","content":"I\'m doing great"},"done":false}\n\n' +
      'data: {"model":"qwen3.5:2b","message":{"role":"assistant","content":"!"},"done":true}\n\n',
      { headers: { 'Content-Type': 'text/event-stream' } }
    );

    const mockResponse2 = new Response(
      'data: {"model":"qwen3.5:2b","message":{"role":"assistant","content":"Your initial question was: How are you today?"},"done":true}\n\n',
      { headers: { 'Content-Type': 'text/event-stream' } }
    );
  ...
  it('handles streaming response correctly', async () => {
    ...
    const mockResponse = new Response(
      'data: {"model":"qwen3.5:2b","message":{"role":"assistant","content":"Hello"},"done":false}\n\n' +
      'data: {"model":"qwen3.5:2b","message":{"role":"assistant","content":" World"},"done":false}\n\n' +
      'data: {"model":"qwen3.5:2b","message":{"role":"assistant","content":"!"},"done":true}\n\n',
      { headers: { 'Content-Type': 'text/event-stream' } }
    );
  ...
  it('verifies conversation history is included in subsequent requests', async () => {
    ...
    const responses = [
      new Response(
        'data: {"model":"qwen3.5:2b","message":{"role":"assistant","content":"First response"},"done":true}\n\n',
        { headers: { 'Content-Type': 'text/event-stream' } }
      ),
      new Response(
        'data: {"model":"qwen3.5:2b","message":{"role":"assistant","content":"Second response"},"done":true}\n\n',
        { headers: { 'Content-Type': 'text/event-stream' } }
      ),
      new Response(
        'data: {"model":"qwen3.5:2b","message":{"role":"assistant","content":"Third response"},"done":true}\n\n',
        { headers: { 'Content-Type': 'text/event-stream' } }
      )
    ];
  ...
  it('should include temperature in chat request', async () => {
    // given
    const mockResponse = new Response(
      'data: {"model":"qwen3.5:2b","message":{"role":"assistant","content":"Hello"},"done":true}\n\n',
      { headers: { 'Content-Type': 'text/event-stream' } }
    );
    vi.mocked(ollama.chat).mockResolvedValue(mockResponse);
    const { result } = await renderChatHook();
